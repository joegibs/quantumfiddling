...
mut = []
    for i in interval
        print("\n meas_p $i \n")
        svn,tri_mut =do_trials(N,steps,i,40,[0],"None",false)
...
p = plot([0.0:0.2:1...],decays,title=string("Bip_ent Gat: 2haar, varying meas_p"), label=string.(transpose([6:2:14...])), linewidth=3,xlabel = "Meas_P", ylabel = L"$\textbf{S_{vn}}(L/2)$")
